...
def build_path(wp_path):
	...
	quaternion = tf.transformations.quaternion_conjugate(quaternion)
	quaternion = tf.transformations.quaternion_multiply(quaternion, quaternion) #normalizar

        rospy.loginfo('Quaternion: {}'.format(', '.join(map(str, quaternion))))
